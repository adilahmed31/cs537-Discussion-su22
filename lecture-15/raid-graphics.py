...
class Raid:
    def __init__(self, mapping, addr_desc, addr, disk_count, seek_speed, seed, balance, read_fraction, window, animate_delay):
        ...
        for index in range(len(self.request_queue)):
            ...
            logical = request.GetLogicalAddress()
            self.request_queue_box_ids.append(self.canvas.create_rectangle(x-sz,y-sz,x+sz,y+sz,outline=''))
        ...
        for i in range(self.disk_count):
            ...
            self.canvas.create_rectangle(x+50,510,x,530, fill='green', outline='')
            self.count_reads_id[i] = self.canvas.create_text(x-20, 520, text='R:0', anchor='c', font=font,fill='black')
            self.count_writes_id[i] = self.canvas.create_text(x+20, 520, text='W:0', anchor='c', font=font, fill='black')
            x += disk_width + 20
    ...
    def StartWrite(self, disk, offset, logical_address, request, queue_index):
        ...
        self.StartRequest(disk, offset, logical_address, request, queue_index, 'green')
        return
